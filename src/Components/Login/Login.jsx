...
import { useLoginApiMutation } from '../../Features/Auth/AuthApi';
...
const Login = () => {
  const [loginApi,{data,isLoading,isSuccess,error}]=useLoginApiMutation()
  ...
  useEffect(()=>{
    if(data?.success){
     dispatch(login({user:data?.user,accessToken:data?.accessToken,refreshToken:data?.refreshToken})) 
          ...
          <div className="md:max-w-md w-full sm:px-6 py-4">
            <form  onSubmit={formik?.handleSubmit}>
              <div className="mb-12">
                ...
                <p className="text-sm mt-4 ">Don't have an account <NavLink to="/register" className="text-blue-600 font-semibold hover:underline ml-1 whitespace-nowrap">Register here</NavLink></p>
              </div>
              <div>
                <label className="text-xs block mb-2">Email</label>
                <div className="relative flex items-center">
                  <input onChange={formik.handleChange} name="email" type="text" required className="w-full text-sm border-b border-gray-300 focus:border-[#333] px-2 py-3 outline-none" placeholder="Enter email" />
                ...
                {formik.touched && formik.errors?.email && <p className='text-red-500 font-Poppins pt-1'>{formik?.errors?.email}</p>}
              </div>
              <div className="mt-8">
                <label className="text-xs block mb-2">Password</label>
                <div className="relative flex items-center">
                  <input onChange={formik.handleChange} name="password" type="password" required className="w-full text-sm border-b border-gray-300 focus:border-[#333] px-2 py-3 outline-none" placeholder="Enter password" />
              ...
              {formik.touched && formik.errors?.password && <p className='text-red-500 font-Poppins pt-1'>{formik?.errors?.password}</p>}
